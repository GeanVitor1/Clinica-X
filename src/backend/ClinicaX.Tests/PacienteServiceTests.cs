...
public class PacienteServiceTests
{
    ...
    public PacienteServiceTests()
    {
        ...
        _service = new PacienteService(_repoMock.Object, _agendamentoRepoMock.Object, _eventoRepoMock.Object, _uowMock.Object);
    }

    [Fact]
    public async Task GetByIdAsync_DeveRetornarPaciente_QuandoEncontrado()
    {
        var paciente = new Paciente { Id = _pacienteId, Nome = "João", Cpf = "123" };
        _repoMock.Setup(r => r.GetByIdAsync(_pacienteId, default)).ReturnsAsync(paciente);

        var result = await _service.GetByIdAsync(_pacienteId);
    ...
    {
        _repoMock.Setup(r => r.GetByIdAsync(It.IsAny<Guid>(), default)).ReturnsAsync((Paciente?)null);
    ...
    {
        var request = new CreatePacienteRequest("Maria", "456", "99999", null, null);
    ...
    public async Task UpdateAsync_DeveAtualizarComSucesso()
    {
        ...
        var request = new UpdatePacienteRequest("Novo", "123", "88888", null, null);
        var result = await _service.UpdateAsync(_pacienteId, request);
    ...
    {
        var paciente = new Paciente { Id = _pacienteId, Ativo = true };
        _repoMock.Setup(r => r.GetByIdAsync(_pacienteId, default)).ReturnsAsync(paciente);
        _uowMock.Setup(u => u.SaveChangesAsync(default)).ReturnsAsync(1);

        var result = await _service.DeleteAsync(_pacienteId);
