...
import { AgendamentoService, Agendamento, CreateAgendamentoRequest } from '../../services/agendamento.service';
...
@Component({
      ...
      @if (view() === 'day') {
        <div class="day-view">
          @for (slot of daySlots(); track slot.hora) {
            <div class="slot" [class.livre]="slot.livre" [class.ocupado]="!slot.livre"
              ...
              @if (slot.livre) {
                ...
                <div class="agendamento-info" [style.border-left-color]="slot.agendamento?.cor || '#14b8a6'">
                  <strong>{{ slot.agendamento?.pacienteNome }}</strong>
                  <span>{{ slot.agendamento?.servicoNome }}</span>
                </div>
                <button type="button" class="btn-cancel-slot"
                  (click)="abrirCancelar(slot.agendamento!.id); $event.stopPropagation()"
                  title="Cancelar agendamento">✕</button>
              }
            </div>
          }
        </div>
              ...
              @for (slot of day.slots; track slot.hora) {
                <div class="slot" [class.livre]="slot.livre" [class.ocupado]="!slot.livre"
                  ...
                  @if (slot.livre) {
                    ...
                    <button type="button" class="btn-cancel-slot btn-cancel-slot--sm"
                      (click)="abrirCancelar(slot.agendamento!.id); $event.stopPropagation()"
      ...
      @if (view() === 'month') {
        <div class="month-view">
          <div class="month-header">
            @for (d of ['Dom','Seg','Ter','Qua','Qui','Sex','Sáb']; track d) { <span>{{ d }}</span> }
          </div>
          @for (week of monthWeeks(); track week[0]?.date) {
            <div class="month-week">
              @for (day of week; track day.date) {
                <div class="month-day" [class.other-month]="day.otherMonth" [class.today]="day.today">
                  <span class="day-num">{{ day.num }}</span>
                  @for (s of day.slots; track s.hora + (s.agendamento?.id || '')) {
                    @if (!s.livre) {
                      <div class="mini-event" [style.background]="s.agendamento?.cor || '#14b8a6'"
                        (click)="abrirCancelar(s.agendamento!.id)"
                        title="Clique para cancelar">{{ s.agendamento?.pacienteNome }}</div>
                    }
                  }
                </div>
              }
        ...
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <form (ngSubmit)="createAgendamento()" class="agenda-form">
            ...
            <div class="form-actions">
              <button type="button" class="btn-secondary" (click)="showNewForm = false">Voltar</button>
              <app-btn-submit label="Agendar" loadingLabel="Agendando..." doneLabel="Agendado!" [state]="saveState()" />
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="modal-overlay" (click)="showCancelForm = false">
        <div class="modal" (click)="$event.stopPropagation()">
          ...
          <form (ngSubmit)="confirmarCancelamento()" class="agenda-form">
            ...
            <div class="form-actions">
              <button type="button" class="btn-secondary" (click)="showCancelForm = false">Voltar</button>
              <button type="submit" class="btn-danger" [disabled]="!cancelMotivo.trim()">Confirmar Cancelamento</button>
            </div>
          </form>
        </div>
      </div>
  ...
  styles: [`
    ...
    .page-header { display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px; margin-bottom: 16px; }
    ...
    .view-controls { display: flex; gap: 2px; background: var(--clx-surface-3); border-radius: var(--clx-radius-md); padding: 3px; }
    ...
    .view-controls button.active { background: var(--clx-accent); color: #fff; }

    .nav-controls { display: flex; align-items: center; gap: 12px; margin-bottom: 20px; }
    .nav-controls button {
      padding: 7px 10px;
      border: 1px solid var(--clx-border-strong);
      border-radius: var(--clx-radius-sm);
    ...
    .nav-controls button:hover { border-color: var(--clx-accent); color: var(--clx-accent); }
    .current-period { font-weight: 650; color: var(--clx-text-primary); min-width: 200px; text-align: center; font-size: 0.9rem; }

    .day-view { display: flex; flex-direction: column; gap: 3px; }
    .slot {
      display: flex;
      align-items: center;
      gap: 12px;
    ...
    .slot.ocupado { cursor: grab; }
    .slot.ocupado:active { cursor: grabbing; }
    .slot:hover { border-color: var(--clx-border-strong); }
    ...
    .agendamento-info strong { color: var(--clx-text-primary); font-size: 0.82rem; }
    .agendamento-info span { color: var(--clx-text-secondary); font-size: 0.74rem; }

    .slot-name { flex: 1; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 0.78rem; }
    ...
    .btn-cancel-slot:hover { background: var(--clx-error); color: #fff; }
    .btn-cancel-slot--sm { width: 20px; height: 20px; font-size: 0.6rem; }
    ...
    .week-day .slot { padding: 6px 8px; font-size: 0.72rem; gap: 4px; }

    .month-header { display: grid; grid-template-columns: repeat(7, 1fr); text-align: center; font-size: 0.74rem; color: var(--clx-text-tertiary); padding: 8px 0; font-weight: 600; }
    .month-week { display: grid; grid-template-columns: repeat(7, 1fr); border-top: 1px solid var(--clx-border); }
    .month-day { min-height: 85px; padding: 4px; border-right: 1px solid var(--clx-border); font-size: 0.74rem; background: var(--clx-surface-1); }
    ...
    @media (max-width: 768px) {
      .week-view { grid-template-columns: repeat(7, minmax(100px, 1fr)); overflow-x: auto; }
      .month-day { min-height: 55px; }
...
export class AgendaViewComponent implements OnInit {
  ...
  currentPeriodLabel = computed(() => {
    ...
    switch (this.view()) {
      case 'day': return d.toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' });
      case 'week': return `Semana de ${d.toLocaleDateString('pt-BR', { day: 'numeric' })}`;
      case 'month': return d.toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
    }
  });
  ...
  monthWeeks = computed(() => {
    ...
    const weeks: { date: string; num: number; otherMonth: boolean; today: boolean; slots: Slot[] }[][] = [];
    let week: typeof weeks[0] = [];
    const startPad = d.getDay();
    for (let i = 0; i < startPad; i++) {
      const prev = new Date(d);
      prev.setDate(prev.getDate() - (startPad - i));
      week.push({ date: prev.toISOString(), num: prev.getDate(), otherMonth: true, today: false, slots: [] });
    }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      week.push({
        date: date.toISOString(), num: i, otherMonth: false,
  ...
  onDrop(e: DragEvent, targetSlot: Slot) {
    ...
    if (this.draggedSlot.agendamento.id &&
        new Date(this.draggedSlot.agendamento.dataHoraInicio).getTime() === novaData.getTime()) {
      this.draggedSlot = null;
      return;
    }

    this.agendamentoService.remarcar(this.draggedSlot.agendamento.id, {
  ...
  confirmarCancelamento() {
    ...
    this.agendamentoService.cancelar(this.cancelAgendamentoId, { motivoCancelamento: this.cancelMotivo.trim() }).subscribe({
      next: () => {
