...
@Component({
  selector: 'app-transcricoes-page',
  standalone: true,
    ...
    <div class="page">
      <header class="page-header">
        <div class="header-left">
          <h1>Transcrição de consultas</h1>
          <span class="header-subtitle">Texto e resumo das consultas</span>
        </div>
        <div class="header-actions">
          <button class="btn-primary" type="button" (click)="toggleForm()">
            ...
            {{ showForm ? 'Fechar' : 'Nova transcrição' }}
          </button>
        </div>
      </header>
      ...
      <div class="kpi-grid">
        <div class="kpi-card">
          ...
          <div class="kpi-info">
            ...
            <span class="kpi-sub">transcrições</span>
          </div>
        </div>
        <div class="kpi-card">
        ...
        <div class="kpi-card">
          <div class="kpi-icon kpi-icon--processing">
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
            </svg>
          </div>
          <div class="kpi-info">
            <span class="kpi-label">Em processamento</span>
            <span class="kpi-value">{{ processando() }}</span>
          </div>
        </div>
        <div class="kpi-card">
          ...
          <div class="kpi-info">
            <span class="kpi-label">Duração total</span>
            <span class="kpi-value kpi-value--sm">{{ duracaoTotal() }}</span>
          </div>
        </div>
      </div>
      ...
      @if (showForm) {
        <div class="panel">
          <div class="panel-header">
            <span class="panel-dot" style="background: var(--clx-rose);"></span>
          ...
          <div class="form-grid">
            ...
            <label>Duração (segundos)
              <input type="number" [(ngModel)]="form.duracaoSegundos" min="0" placeholder="Ex: 420" />
            </label>
            <label class="fw">Texto da consulta *
              <textarea [(ngModel)]="form.texto" placeholder="Transcrição completa da consulta..."></textarea>
            </label>
          </div>
      ...
      <div class="panel">
        @if (items().length === 0) {
          <div class="empty-wrap">
            ...
            <h4>Nenhuma transcrição encontrada</h4>
            <p>Clique em <strong>Nova transcrição</strong> para registrar a primeira transcrição.</p>
          </div>
        } @else {
          <div class="table-toolbar">
            <div class="search-box">
              ...
              <input type="text" class="search-input" placeholder="Buscar por resumo ou texto..." [ngModel]="busca()" (ngModelChange)="busca.set($event)">
              @if (busca()) {
            ...
            <table class="trans-table">
              ...
              <tbody>
                @for (item of itemsFiltrados(); track item.id) {
                  <tr>
                    <td class="td-date" data-label="Data">{{ item.data | date:"dd/MM/yyyy HH:mm" }}</td>
                    <td class="td-resumo" data-label="Resumo">{{ item.resumo || '—' }}</td>
                    <td class="td-duration" data-label="Duração">{{ formatarDuracao(item.duracaoSegundos) }}</td>
                    <td data-label="Status">
                      <span class="status-badge" [class.status-badge--done]="item.status === 'Concluida'"
                            [class.status-badge--processing]="item.status === 'Processando'"
                            [class.status-badge--error]="item.status === 'Erro'">
          ...
          @if (itemsFiltrados().length === 0) {
            <p class="no-results">Nenhuma transcrição encontrada para "{{ busca() }}".</p>
          }
        }
      </div>
    </div>
  ...
  styles: [`
    ...
    .kpi-icon--done { background: var(--clx-teal-muted); color: var(--clx-teal); }
    .kpi-icon--processing { background: var(--clx-amber-muted); color: var(--clx-amber); }
    .kpi-icon--duration { background: var(--clx-purple-muted); color: var(--clx-purple); }
    ...
    .td-resumo { max-width: 360px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; color: var(--clx-text-secondary); }
    .td-duration { font-variant-numeric: tabular-nums; font-weight: 600; color: var(--clx-text-primary); }
    ...
    @media (max-width: 700px) {
      ...
      .trans-table { display: block; width: 100%; }
      .trans-table tbody { display: block; }
      .trans-table tbody tr {
        display: block;
        border: 1px solid var(--clx-border);
        border-radius: 8px;
      ...
      .trans-table tbody td:last-child { border-bottom: none; }
      .trans-table tbody td::before {
      ...
      .trans-table tbody td.td-right,
      .trans-table tbody td[class*="action"] {
    ...
    @media (max-width: 450px) {
      ...
      .trans-table tbody td { font-size: 0.72rem; }
    }
  `],
})
export class TranscricoesPageComponent implements OnInit {
  private readonly api = inject(ModulosApiService);
  private readonly toast = inject(ToastService);
  private readonly pacientesApi = inject(PacienteService);
  readonly auth = inject(AuthService);

  items = signal<TranscricaoItem[]>([]);
  ...
  processando = computed(() => this.items().filter(i => i.status === 'Processando').length);

  duracaoTotal = computed(() => {
    const total = this.items().reduce((s, i) => s + (i.duracaoSegundos || 0), 0);
    return this.formatarDuracao(total);
  });
  ...
  itemsFiltrados = computed(() => {
    ...
    if (busca) {
      result = result.filter(i =>
        (i.resumo && i.resumo.toLowerCase().includes(busca)) ||
  ...
  private resetForm() {
    ...
    this.form = { pacienteId: firstPac, texto: '', duracaoSegundos: null };
  }

  load() {
    this.api.listTranscricoes().subscribe({
  ...
  salvar() {
    const f = this.form;
    if (!f.pacienteId || !f.texto) {
      this.toast.show('error', 'Preencha todos os campos obrigatórios');
      return;
    }
    this.api.createTranscricao(f).subscribe({
      next: () => {
        this.toast.show('success', 'Transcrição salva com sucesso');
        this.showForm = false;
        this.load();
      },
